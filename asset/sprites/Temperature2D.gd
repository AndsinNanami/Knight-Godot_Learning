...
#变温速率
var _speed_by_change_temperature:int = 5
#升温还是降温
...
func _ready():
	_current_temperature = _default_temperature
...
func _physics_process(delta):
	...
	if _current_temperature > _environment_temp:
		_is_be_heated = false
		_change_temperature()
	elif _current_temperature < _environment_temp:
		_is_be_heated = true
		_change_temperature()

func _change_temperature():
	if _is_be_heated:
		_current_temperature += _speed_by_change_temperature
		prints("现在温度：",_current_temperature)
	else:
		_current_temperature -= _speed_by_change_temperature
		prints("现在温度：",_current_temperature)
